...
class PGPApplication:
    ...
    def setup_ui(self):
        ...
        button_style = {
            ...
            'bg': BUTTON_COLORS['fill'],              # Button fill color
            'fg': BUTTON_COLORS['text'],              # Button text color
            'activebackground': BUTTON_COLORS['active'],  # Same fill on click
            'activeforeground': BUTTON_COLORS['text'],    # Same text on click
            'borderwidth': 4,
            'relief': 'solid',
            'cursor': 'hand2',
            'highlightthickness': 4,
            'highlightbackground': BUTTON_COLORS['border'],  # Black border
            'highlightcolor': BUTTON_COLORS['border']        # Black border when focused
        }
    ...
    def create_info_section(self, parent):
        ...
        why_text = tk.Label(parent,
            text=(
                "The internet was built on principles of freedom, privacy, and decentralization. Today, these principles\n"
                "are under constant assault. Mass surveillance, corporate data harvesting, and government overreach threaten\n"
                "our fundamental right to private communication. PGP (Pretty Good Privacy) represents one of the last bastions\n"
                "of truly private, encrypted communication that cannot be intercepted, backdoored, or compromised by third parties.\n\n"
                
                "Unlike proprietary messaging apps that can be compelled to hand over data or implement backdoors, PGP is:\n"
                ...
                "In an era where privacy is treated as a product to be sold rather than a right to be protected,\n"
                "PGP empowers individuals to take back control of their communications. It's not just about having\n"
                "something to hide—it's about preserving the fundamental human right to privacy in digital spaces."
            ),
            font=('Courier New', 11),
            fg=self.colors['text_dark_grey'],
            bg=self.colors['bg'],
        ...
        tech_text = tk.Label(parent,
            text=(
                ...
                "The free internet depends on tools like these—and people like you who use them."
            ),
            font=('Courier New', 11),
            fg=self.colors['text_dark_grey'],
            bg=self.colors['bg'],
    ...
    def create_key_dialog(self):
        ...
        def create_key():
            ...
            try:
                ...
                status_label.config(text=f"Generating {key_size}-bit RSA keypair... Please wait...", foreground='orange')
                dialog.update()
                ...
                status_label.config(text=f"✓ Keys generated and saved successfully!", foreground='green')
        ...
        def verify():
            ...
            try:
                ...
                if verified:
                    ...
                    result_text.insert(tk.END, f"This message was signed by:\n")
                    ...
                    result_text.insert(tk.END, f"Verifying with key: {selected_key_info['name']} [{fingerprint[-16:]}]\n\n")
                    
                    if 'error' in info:
                        result_text.insert(tk.END, f"Error: {info['error']}\n\n")
                    
                    result_text.insert(tk.END, "Possible reasons:\n")
                    result_text.insert(tk.END, "  • Wrong public key selected (message was signed with a different key)\n")
